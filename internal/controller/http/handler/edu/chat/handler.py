...
class EduChatController(interface.IEduChatController):
    def __init__(
            self,
        ...
        self.edu_chat_service = edu_chat_service
    ...
    async def send_message_to_interview_expert(self, body: SendMessageToExpert):
        with self.tracer.start_as_current_span(
            ...
            try:
                llm_response = await self.edu_chat_service.send_message_to_interview_expert(
                    body.account_id,
                    body.text
                )
    ...
    async def send_message_to_teacher(self, body: SendMessageToExpert):
        with self.tracer.start_as_current_span(
            ...
            try:
                llm_response = await self.edu_chat_service.send_message_to_teacher(
                    body.account_id,
                    body.text
                )
    ...
    async def send_message_to_test_expert(self, body: SendMessageToExpert):
        with self.tracer.start_as_current_span(
            ...
            try:
                llm_response = await self.edu_chat_service.send_message_to_test_expert(
                    body.account_id,
                    body.text
                )
